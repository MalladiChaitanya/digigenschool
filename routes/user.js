...
exports.getProfileDetails = function (req, res) {
    var data = {};
    console.log(req.body);
    db.assignmentsModel.find({}, { _id: 0 }, function (err, success) {
        if (success) {
            console.log(success);
            data.assignments = success;
            data.status = 201;
            res.json(prepareProfileData(data.assignments));
        } else {
            console.log("failure", err);
            data.status = 401;
            res.json(data);
...
function prepareProfileData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        if (!user[data[i].username]) {
            ...
            user[data[i].username].assignments = [];
            user[data[i].username]["assignments"].push(data[i]);
